...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['logo'])) {
    if ($_FILES['logo']['error'] == 0) {
        $targetDir = "logo/"; // Directory where the logo will be saved
        $fileType = strtolower(pathinfo($_FILES['logo']['name'], PATHINFO_EXTENSION));
        $targetFilePath = $targetDir . "logo." . $fileType; // Always use "logo" as filename

        // Allow certain file formats
        $allowedTypes = array('jpg', 'png', 'jpeg', 'gif');
        if (in_array($fileType, $allowedTypes)) {
            // Delete existing logo files
            $existingFiles = glob($targetDir . "logo.*");
            ...
            if (move_uploaded_file($_FILES['logo']['tmp_name'], $targetFilePath)) {
                $_SESSION['success_message'] = "Logo başarıyla güncellendi.";
            } else {
                $_SESSION['error_message'] = "Logo yüklenirken bir hata oluştu.";
            }
        } else {
            $_SESSION['error_message'] = "Geçersiz dosya formatı. Lütfen JPG, JPEG, PNG veya GIF formatında bir dosya yükleyin.";
        }
    } else {
        $_SESSION['error_message'] = "Dosya yüklenmedi veya bir hata oluştu.";
    }
}
...
<body>
                ...
                <div class="page-header">
                    <div class="d-flex align-items-center gap-3 justify-content-between w-100">
                        <div class="d-flex align-items-center gap-3">
                            ...
                            <h2 class="mb-0 text-center">Sistem Logları</h2>
                        </div>
                        <button class="btn btn-light" data-bs-toggle="modal" data-bs-target="#settingsModal">
                            <i class="bi bi-gear text-primary" style="font-size: 24px;"></i>
                        </button>
                    </div>
                </div>
    ...
    <div class="modal fade" id="settingsModal" tabindex="-1" aria-labelledby="settingsModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="settingsModalLabel">Logo Yükle</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="logoUploadForm" method="POST" enctype="multipart/form-data">
                        <div class="mb-3">
                            <label for="logo" class="form-label">Logo Seçin</label>
