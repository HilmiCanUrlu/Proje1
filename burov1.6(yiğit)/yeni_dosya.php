...
try {
    $personel_query = "SELECT personel_id, ad,soyad FROM personel ORDER BY ad ASC";
    $personel_stmt = $db->prepare($personel_query);
    $personel_stmt->execute();
    $personel = $personel_stmt->fetchAll(PDO::FETCH_ASSOC);
    
    // Debug için müşteri sayısını ve verileri yazdır
    error_log("Personel Sayısı: " . count($personel));
    error_log("Personel Verileri: " . print_r($personel, true));
    
    if (empty($personel)) {
        error_log("Dikkat: personel tablosu boş!");
    }
} catch(PDOException $e) {
    error_log("personel sorgusu hatası: " . $e->getMessage());
}

// İşlem türlerini veritabanından çek
try {
    $islem_query = "SELECT id, islem_turu FROM d_islem ORDER BY islem_turu ASC";
    $islem_stmt = $db->prepare($islem_query);
    $islem_stmt->execute();
    $islem_turleri = $islem_stmt->fetchAll(PDO::FETCH_ASSOC);
    
    error_log("İşlem Türü Sayısı: " . count($islem_turleri));
    error_log("İşlem Türleri: " . print_r($islem_turleri, true));
} catch(PDOException $e) {
    error_log("İşlem türü sorgusu hatası: " . $e->getMessage());
}
                ...
                <div class="card">
                    <div class="card-body">
                        <form method="POST" action="">
                            ...
                            <div class="col-md-6">
                                    <label class="form-label required">Sorumlu Personel</label>
                                    <div class="input-group">
                                        <select name="personel_id" class="form-select" required>
                                            ...
                                            ?>
                                                <option value="<?php echo htmlspecialchars($person['personel_id']); ?>">
                                                    <?php 
                                                    echo htmlspecialchars($person['ad']) . 
                                                         ' (' . htmlspecialchars($person['soyad']) . ')'; 
                                                    ?>
                                                </option>
                                            <?php 
                                                endforeach; 
    ...
    fetch('get_mahalleler.php')
        ...
        .then(data => {
            ...
            <?php foreach ($islem_turleri as $islem): ?>
                islemTuruSelect.innerHTML += `<option value="<?php echo htmlspecialchars($islem['islem_turu']); ?>"><?php echo htmlspecialchars($islem['islem_turu']); ?></option>`;
            <?php endforeach; ?>
