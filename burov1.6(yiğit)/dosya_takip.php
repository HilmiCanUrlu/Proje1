...
// Sorumlu personel filtresi ekle
if (isset($_GET['sorumlu_personel']) && !empty($_GET['sorumlu_personel'])) {
    $where .= " AND d.personel_id = :sorumlu_personel";
    $params[':sorumlu_personel'] = $_GET['sorumlu_personel'];
}

// Dosyaları getir
$query = "SELECT d.*, m.musteri_adi, p.ad, p.soyad 
          FROM dosyalar d 
...
$personel_query = "SELECT personel_id, ad, soyad FROM personel ORDER BY ad, soyad";
$personel_stmt = $db->query($personel_query);
$personel_listesi = $personel_stmt->fetchAll(PDO::FETCH_ASSOC);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete') {
    ...
    if (!isset($_POST['dosya_id']) || empty($_POST['dosya_id'])) {
        $response['message'] = 'Dosya ID gerekli';
    } else {
        try {
                ...
                <div class="row mb-4">
                    <div class="col-md-4">
                        <div class="stats-card bg-white">
                            <h6><i class="bi bi-folder text-primary me-2"></i>Tapu Dosyaları</h6>
                            <h2><?php echo $shkm_count; ?></h2>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="stats-card bg-white">
                            <h6><i class="bi bi-folder text-success me-2"></i>Belediye Dosyaları</h6>
                            <h2><?php echo $lihkab_count; ?></h2>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="stats-card bg-white">
                            <h6><i class="bi bi-folder text-warning me-2"></i>Tamamlanan Dosyalar</h6>
...
<div class="card mb-4">
  <div class="card-body">
    ...
    <form method="GET" class="row g-3">
      ...
      <div class="col-md-4">
        ...
        <select name="sorumlu_personel" class="form-select">
          ...
          <?php foreach($personel_listesi as $p): ?>
            <option value="<?php echo $p['personel_id']; ?>"
              <?php echo (isset($_GET['sorumlu_personel']) && $_GET['sorumlu_personel']==$p['personel_id'])? 'selected':''; ?>>
      ...
      <div class="col-md-4">
        ...
        <select name="durum" class="form-select">
          <option value="">Seçiniz</option>
          <option value="aktif" <?php echo (isset($_GET['durum']) && $_GET['durum']==='aktif')? 'selected':''; ?>>Aktif</option>
          <option value="pasif" <?php echo (isset($_GET['durum']) && $_GET['durum']==='pasif')? 'selected':''; ?>>Pasif</option>
        </select>
      </div>

      <!-- Gelişmiş Filtreleri Göster/Gizle Butonu -->
      <div class="col-12">
        <button type="button"
          ...
          <i class="bi bi-chevron-down me-1"></i>Gelişmiş Filtreler
        </button>
      </div>
        ...
        <div class="row g-3">
          <div class="col-md-4">
            ...
            <select name="dosya_turu" class="form-select">
              <option value="">Seçiniz</option>
              <?php foreach($dosya_turleri as $tur): ?>
                <option value="<?php echo htmlspecialchars($tur); ?>"
                  <?php echo (isset($_GET['dosya_turu']) && $_GET['dosya_turu']===$tur)? 'selected':''; ?>>
          ...
          <div class="col-md-4">
            ...
            <select name="islem_turu" class="form-select">
              ...
              <?php foreach($islem_turleri as $tur): ?>
                <option value="<?php echo htmlspecialchars($tur); ?>"
                  <?php echo (isset($_GET['islem_turu']) && $_GET['islem_turu']===$tur)? 'selected':''; ?>>
                  <?php echo htmlspecialchars($tur); ?>
                </option>
              <?php endforeach; ?>
            </select>
            ...
            <select name="il" class="form-select">
              ...
              <?php foreach($iller as $il): ?>
                <option value="<?php echo htmlspecialchars($il); ?>"
                  <?php echo (isset($_GET['il']) && $_GET['il']===$il)? 'selected':''; ?>>
                  <?php echo htmlspecialchars($il); ?>
                </option>
              <?php endforeach; ?>
            </select>
          </div>
          ...
          <div class="col-md-4">
            ...
            <select name="dosya_durumu" class="form-select">
              <option value="">Seçiniz</option>
              <?php foreach($dosya_durumlari as $d): ?>
                <option value="<?php echo htmlspecialchars($d); ?>"
                  <?php echo (isset($_GET['dosya_durumu']) && $_GET['dosya_durumu']===$d)? 'selected':''; ?>>
      ...
      <div class="col-12">
        ...
        <a href="dosya_takip.php" class="btn btn-secondary ms-2">
          <i class="bi bi-arrow-repeat me-1"></i>Sıfırla
        </a>
      </div>
    </form>
  </div>
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="section-title">
                            ...
                            Dosya Listesi
                            <span class="badge bg-primary ms-2"><?php echo count($dosyalar); ?> kayıt</span>
                        </div>
                            ...
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <?php if(isset($dosya['ad']) && isset($dosya['soyad'])): ?>
                                                <?php echo htmlspecialchars($dosya['ad'] . ' ' . $dosya['soyad']); ?>
                                            <?php else: ?>
                                        ...
                                        <td>
                                            <span class="badge <?php 
                                                echo $dosya['dosya_durumu'] == 'Tamamlandı' ? 'bg-success' : 
                                                    ...
                                                    ($dosya['dosya_durumu'] == 'Belediyede' || $dosya['dosya_durumu'] == 'Kadastroda' ? 'bg-info' : 'bg-primary')); 
                                            ?>">
                                        ...
                                        <td>
                                            <div class="btn-group btn-group-sm">
                                                ...
                                                <button class="btn btn-success" onclick="showMuhasebeDetay(<?php echo $dosya['dosya_id']; ?>)">
                                                    <i class="bi bi-cash me-1"></i>Muhasebe
                                                </button>
                                                <button class="btn btn-danger" onclick="showSilmeOnay(<?php echo $dosya['dosya_id']; ?>)">
    ...
    <!-- Dosya Detay Modal -->
    <div class="modal fade" id="dosyaDetayModal" tabindex="-1">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row mb-4">
                        <div class="col-md-6">
                            <div class="card">
                                ...
                                <div class="card-body" id="dosyaBilgileri">
                        ...
                        <div class="col-md-6">
                            <div class="card">
                                ...
                                <div class="card-body" id="musteriBilgileri">
                                    <!-- JavaScript ile doldurulacak -->
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body" id="islemlerListesi">
                            <!-- JavaScript ile doldurulacak -->
                        </div>
                    </div>
                </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-danger" id="silmeOnayBtn">Sil</button>
                </div>
            </div>
        </div>
    </div>
    ...
    fetch('get_mahalleler.php')
        ...
        .then(data => {
            console.log('Data received from get_mahalleler.php:', data);
            ...
            // Check if data contains the needed fields
            if (!data.dosya_durumlari) console.error('Missing dosya_durumlari field in API response');
            if (!data.durum_secenekleri) console.error('Missing durum_secenekleri field in API response');
            ...
            data.dosya_durumlari.forEach(durum => {
                if (durum === 'Tapuda') {
                    dosyaDurumuSelect.innerHTML += `<option value="Kadastroda">Kadastroda</option>`;
                } else {
        ...
        .catch(error => {
            console.error('Error fetching data from get_mahalleler.php:', error);
        });
        ...
        fetch(`get_dosya_detay.php?dosya_id=${dosyaId}`)
            ...
            .then(data => {
                document.getElementById('dosyaBilgileri').innerHTML = `
                    ...
                    <p><strong>Dosya Durumu:</strong> <span class="badge ${data.dosya.dosya_durumu === 'Tamamlandı' ? 'bg-success' : (data.dosya.dosya_durumu === 'Beklemede' ? 'bg-warning' : (data.dosya.dosya_durumu === 'Belediyede' || data.dosya.dosya_durumu === 'Kadastroda' ? 'bg-info' : 'bg-primary'))}">${data.dosya.dosya_durumu || 'Hazırlandı'}</span></p>
    ...
    // URL parametrelerinden muhasebe değerini kontrol et
    document.addEventListener('DOMContentLoaded', function() {
        const urlParams = new URLSearchParams(window.location.search);
        const muhasebeParam = urlParams.get('muhasebe');
        
        if (muhasebeParam) {
            // Muhasebe modalını aç
            showMuhasebeDetay(muhasebeParam);
        }
    });

    // Silme işlemi için değişken
    let silinecekDosyaId = null;

    // Silme butonuna tıklandığında
    function showSilmeOnay(dosyaId) {
        silinecekDosyaId = dosyaId;
        new bootstrap.Modal(document.getElementById('silmeOnayModal')).show();
    }

    // Silme onay butonuna tıklandığında
    document.getElementById('silmeOnayBtn').addEventListener('click', function() {
        if (silinecekDosyaId) {
            const formData = new FormData();
            formData.append('action', 'delete');
            formData.append('dosya_id', silinecekDosyaId);
            
            fetch('dosya_takip.php', {
                method: 'POST',
                body: formData
            })
            .then(response => response.json())
